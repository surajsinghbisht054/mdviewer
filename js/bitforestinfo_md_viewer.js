...
//	('Styles', 'Class') <-- Index Number is Key [Use Class for Bootstrap Features]
	...
	['','my-2 mx-1'], // 2==> Global 
	['','text-white p-3 my-2 bg-dark'], // 3==> Programming Codes 
	['','p-1 px-2 m-2 text-capitalize'], // 4==> Headings
	['','my-3'], // 5==> Horizontal Lines
	['','ml-2'], // 6==> Order List
	['','list-group ml-2'], // 7==> UnorderList
	['',' table  mb-3 mt-1 ml-2 table-striped  table-bordered table-hover bg-light'], // 8==> Table
...
function UnOrderListProcessor(argument){
	...
	for (var i = 0; i < argument.length; i++) {
		tmpobj += '<li class="list-group-item">{}</li>'.replace('{}',argument[i])
	}
...
function OrderListProcessor(argument){
	...
	for (var i = 0; i < argument.length; i++) {
		tmpobj += '<li class="border p-3 border-bottom-0">{}</li>'.replace('{}', argument[i])
	}
