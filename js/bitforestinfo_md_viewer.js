...
//	('Styles', 'Class') <-- Index Number is Key [Use Class for Bootstrap Features]
	["", 'border p-1'], // 0== Blockquote
	["", 'ml-5 p-2 border my-4'],	//1== Tab indents
	['','mx-1'], // 2==> Global 
	['','text-white p-3 my-4 bg-dark'], // 3==> Programming Codes 
	['','p-1 px-2 m-2 text-capitalize mt-5'], // 4==> Headings
	['','my-4'], // 5==> Horizontal Lines
	['','list-group ml-2 my-4'], // 6==> Order List
	['','list-group ml-2 my-4'], // 7==> UnorderList
	['',' table  mb-3 mt-1 ml-2 table-striped  table-bordered table-hover bg-light mb-4'], // 8==> Table
	['','text-primary text-capitalize mx-2'], // 9 ==> Links
	['','bg-secondary p-1 text-white'], // 10 ==> Inline Codes
...
function HRRULERPROCESSOR(argument){
	...
	if (argument[1]=='===') {
		ret = '<h1>{}</h1><hr {style} />'.replace('{}', argument[0]);
	}
	else{
...
function UnOrderListProcessor(argument){
	...
	for (var i = 0; i < argument.length; i++) {
		tmpobj += '<li class="list-group-item border-bottom-0">{}</li>'.replace('{}',argument[i])
	}
...
function OrderListProcessor(argument){
	...
	for (var i = 0; i < argument.length; i++) {
		tmpobj += '<li class="border p-2 list-group-item">{}</li>'.replace('{}', argument[i])
	}
